...
// -- assets
import ImgEmpty from '@assets/image/dummy/shipping-empty.svg';
...
const CheckoutShipping = (props) => {
  const { data, onActiveChange, onSelectShipping, show, onClose, profile, hasAddress = true } = props;

  const role = String(profile ?? '').toLowerCase();
  const isMemberOrDealer = role === 'member' || role === 'dealer';
  const showEmpty = isMemberOrDealer && !hasAddress;
  const showList = !isMemberOrDealer || hasAddress;
      ...
      <div className={style.inner}>
        ...
        {showEmpty && (
          <div className={style.bottom}>
            <Empty
              image={ImgEmpty}
              title='Oops, no shipping address yet.'
                ...
                <div
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (typeof onSelectShipping === 'function') onSelectShipping(item);
